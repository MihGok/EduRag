...
class StepikCourseLoader:
    ...
    def __init__(self):
        ...
        # Создаем сессию С прокси для Stepik API
    ...
    # --- МЕТОДЫ ПАРСИНГА И РАБОТЫ С КУРСОМ ---
    ...
    def process_course(self, course: Dict[str, Any], allowed_lesson_ids: Optional[List[int]] = None):
        """
        ИЗМЕНЕНО: Убрана папка курса, секции сохраняются напрямую в корень.
        """
        cid = course.get('id')
        ...
        # Сохраняем метаданные курса в корень (без отдельной папки)
        ...
        for s in sections:
            # Передаем пустую строку как parent_dir - секции будут в корне
    ...
    def process_section(self, section: Dict[str, Any], parent_dir: str, allowed_lesson_ids: Optional[List[int]] = None):
        """
        ИЗМЕНЕНО: Секции создаются напрямую в parent_dir (корне) без промежуточной папки курса.
        """
        sid = section.get('id')
        pos = section.get('position', 0)
        title = self._sanitize_filename(section.get('title', f'Section_{sid}'))
        
        # Создаем папку секции в корне (если parent_dir = ".")
